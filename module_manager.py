...
# to-do 
class ModuleManager(tf.keras.Model):
	...
	self.nl_attention_transform_matrix = tf.Variable(tf.random.uniform([self.out_dim,self.out_dim],dtype=tf.dtypes.float32))	
        # self.self_attention = nn.MultiheadAttention(self.out_dim, MULTI_HEADS, DROPOUT_RATE)
	self.self_attention = tf.keras.layers.MultiHeadAttention(MULTI_HEADS,self.out_dim,dropout=DROPOUT_RATE)
		...
		# to-do
                ...
                #    self.out_dim, self.out_dim, dtype=torch.float, requires_grad=True))
		self.sequence_attention_transform_matrix = tf.Variable(tf.random.uniform([self.out_dim,self.out_dim],dtype=torch.float32))
                # self.code_sequence_multihead_attention = nn.MultiheadAttention(self.out_dim, MULTI_HEADS, DROPOUT_RATE)
		self.code_sequence_multihead_attention = tf.keras.layers.MultiHeadAttention(MULTI_HEADS,self.out_dim,dropout=DROPOUT_RATE)
        if self.attend_code_graph_states:
	    ...
	    self.graph_attention_transform_matrix = tf.Variable(tf.random.uniform([CODE_EMBEDDING_SIZE, self.out_dim],dtype = tf.dtypes.float32))	
            # self.graph_multihead_attention = nn.MultiheadAttention(self.out_dim, MULTI_HEADS, DROPOUT_RATE)
	    self.graph_multihead_attention = tf.keras.layers.MultiHeadAttention(MULTI_HEADS,self.out_dim,dropout = DROPOUT_RATE)

        if self.features:
	    # to-do
            # self.code_features_to_embedding = nn.Linear(CODE_EMBEDDING_SIZE + get_num_code_features(),
            #    CODE_EMBEDDING_SIZE, bias=False)
	    self.code_features_to_embedding = tf.keras.layers.Dense(CODE_EMBEDDING_SIZE, input_shape = (CODE_EMBEDDING_SIZE + get_num_code_features()), use_bias = False)
            # self.nl_features_to_embedding = nn.Linear(
            #    NL_EMBEDDING_SIZE + get_num_nl_features(),
            #    NL_EMBEDDING_SIZE, bias=False)
	    self.nl_features_to_embedding = tf.keras.layers.Dense(NL_EMBEDDING_SIZE,input_shape = (NL_EMBEDDING_SIZE + get_num_nl_features()),use_bias = False)
        ...
        if self.generate:
            ...
            #    self.out_dim, dtype=torch.float, requires_grad=True))
       	    self.encoder_final_to_decoder_initial =  tf.Variable(tf.random.uniform([self.update_encoder_state_size,self.out_dim],dtype=tf.dtypes.float32))
        if self.classify:
	    ...
	    self.attended_nl_encoder_output_layer = tf.keras.layers.Dense(self.out_dim,input_shape = (self.attention_state_size),use_bias = False)
...
#                                            tensorize_graph_method_batch(graph_batch, device, self.max_ast_length)))
		...
		batches.append(UpdateBatchData(tf.convert_to_tensor(code_token_ids, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(code_lengths, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(old_nl_token_ids, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(old_nl_lengths, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(trg_token_ids, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(trg_extended_token_ids, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(trg_lengths, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(invalid_copy_positions, dtype=tf.dtypes.uint8),
					       inp_str_reps,
					       tf.convert_to_tensor(inp_ids, dtype=tf.dtypes.int64),
					       tf.convert_to_tensor(code_features, dtype=tf.dtypes.float32),
					       tf.convert_to_tensor(nl_features, dtype=tf.dtypes.float32),
					       tf.convert_to_tensor(labels, dtype=tf.dtypes.int64),
                                               tensorize_graph_method_batch(graph_batch, device, self.max_ast_length)))
             	curr_idx = end_idx
        return batches
    ...
    def get_encoder_output(self, batch_data, device):
	    ...
	    old_nl_embedded_subtokens = self.nl_features_to_embedding(tf.concat(
                 [old_nl_embedded_subtokens, batch_data.nl_features], axis =-1))
	...
	old_nl_masks = (tf.reshape(tf.range(old_nl_hidden_states.shape[1])) >= batch_data.old_nl_lengths.view(-1, 1)).unsqueeze(1)
		...
		code_embedded_subtokens  = self.code_features_to_embedding(tf.concat(
                 [code_embedded_subtokens, batch_data.code_features], axis =-1))
	    ...
	    code_masks = (tf.reshape(tf.range(code_hidden_state.shape[1])) >= batch_data.code_lengths.view(-1, 1)).unsqueeze(1)
		...
		attention_states = tf.concat([attention_states, compute_attention_states(
                    code_hidden_states, code_masks, old_nl_hidden_states,
                    transformation_matrix=self.sequence_attention_transform_matrix,
                    multihead_attention=self.code_sequence_multihead_attention)], axis=-1)
...
#                    [embedded_nodes, batch_data.graph_batch.node_features], dim=-1))
            	embedded_nodes = self.code_features_to_embedding(tf.concat(
                    [embedded_nodes, batch_data.graph_batch.node_features], dim=-1))
...
#                graph_states.shape[1], device=device).view(1, -1) >= graph_lengths.view(-1, 1)).unsqueeze(1)
	    graph_masks = (tf.reshape(tf.range(graph_states.shape[1])) >= graph_lengths.view(-1, 1)).unsqueeze(1)

            # transformed_graph_states = torch.einsum('ijk,km->ijm', graph_states, self.graph_attention_transform_matrix)
            transformed_graph_states = tf.einsum('ijk,km->ijm', graph_states, self.graph_attention_transform_matrix)
            ...
            attention_states = tf.concat([attention_states, graph_attention_states], axis=-1)     
                
        if self.classify:
            # nl_attended_states = torch.tanh(self.attended_nl_encoder_output_layer(attention_states))
            nl_attended_states = tf.math.tanh(self.attended_nl_encoder_output_layer(attention_states))
...
#                self.encoder_final_to_decoder_initial)
	    encoder_final_state = tf.einsum('ij,jk->ik',
                tf.concat([final_states, old_nl_final_state], axis=-1),
                self.encoder_final_to_decoder_initial)
...
#                encoder_hidden_states.shape[1], device=device).view(1, -1) >= input_lengths.view(-1, 1)).unsqueeze(1)
	    mask = (tf.reshape(tf.range(encoder_hidden_state.shape[1])) >= input_lengths.view(-1, 1)).unsqueeze(1)
