...
class UpdateModule(tf.keras.Model):
...
#                                                index=gold_generation_ids).squeeze(-1)
	gold_generation_logprobs = tf.squeeze(tf.gather_nd(generation_logprobs,gold_generation_ids,batch_dims = -1),axis = -1)
...
#            [gold_generation_logprobs.unsqueeze(-1), gold_copy_logprobs.unsqueeze(-1)], dim=-1), dim=-1)
	gold_logprobs = tf.math.reduce_logsumexp(tf.concat([tf.expand_dims(gold_generation_logprobs，axis = -1),tf.expand_dims(gold_copy_logprobs,axis = -1)],axis = -1),axis = -1)
...
#            device=self.get_device()).unsqueeze(0) >= batch_data.trg_nl_lengths.unsqueeze(-1)-1, 0)
	gold_logprobs = tf.where(tf.expand_dims(tf.range(batch_data.trg_nl_ids[:,1:].shape[-1]),axis=0) >= tf.expand_dims(batch_data.trg_nl_lengths,axis=-1),0) 
...
#            masked_loss_by_example = loss_by_example * batch_data.labels.type(torch.FloatTensor).cuda(self.get_device())
	    masked_loss_by_example = loss_by_example * tf.convert_to_tensor(batch_data.labels, dtype=tf.float32)
...
#            inconsistency_labels = torch.ones([batch_size], dtype=torch.int64, device=self.get_device())
	    inconsistency_labels = tf.ones([batch_size], dtype = tf.int64)
...
#                c.cuda()
	...
	comment_generation_model = load_model(FULL_GENERATION_MODEL_PATH)
