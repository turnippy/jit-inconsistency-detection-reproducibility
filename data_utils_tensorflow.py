# data_utils implementation in tensorflow
...
def tensorize_graph_method_batch(batch, device, max_num_nodes_per_graph):
    ...
    return GraphMethodBatch(
        tf.tensor(batch.graph_ids, dtype=tf.int64, device=device),
        tf.tensor(batch.value_lookup_ids, dtype=tf.int64, device=device),
        tf.tensor(batch.src_type_ids, dtype=tf.int64, device=device),
        tf.tensor(batch.root_ids, dtype=tf.int64, device=device),
        tf.tensor(batch.is_internal, dtype=tf.uint8, device=device),
        batch.edges, batch.num_graphs, batch.num_nodes,
        tf.tensor(batch.node_features, dtype=tf.float32, device=device),
        tf.tensor(node_positions, dtype=tf.int64, device=device),
        tf.tensor(batch.num_nodes_per_graph, dtype=tf.int64, device=device))

class GenerationBatchData(NamedTuple):
    """Stores tensorized batch used in generation model."""
    code_ids: tf.Tensor
    code_lengths: tf.Tensor
    trg_nl_ids: tf.Tensor
    trg_extended_nl_ids: tf.Tensor
    trg_nl_lengths: tf.Tensor
    invalid_copy_positions: tf.Tensor
...
class UpdateBatchData(NamedTuple):
    ...
    code_ids: tf.Tensor
    code_lengths: tf.Tensor
    old_nl_ids: tf.Tensor
    old_nl_lengths: tf.Tensor
    trg_nl_ids: tf.Tensor
    trg_extended_nl_ids: tf.Tensor
    trg_nl_lengths: tf.Tensor
    invalid_copy_positions: tf.Tensor
    input_str_reps: List[List[str]]
    input_ids: List[List[str]]
    code_features: tf.Tensor
    nl_features: tf.Tensor
    labels: tf.Tensor
...
class EncoderOutputs(NamedTuple):
    ...
    masks: tf.Tensor
    encoder_final_state: tf.Tensor
    code_hidden_states: tf.Tensor
    code_masks: tf.Tensor
    old_nl_hidden_states: tf.Tensor
    old_nl_masks: tf.Tensor
    old_nl_final_state: tf.Tensor
    attended_old_nl_final_state: tf.Tensor
